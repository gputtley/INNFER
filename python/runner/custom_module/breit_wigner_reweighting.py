...
class breit_wigner_reweighting():
  ...
  def Run(self):
    ...
    for data_split in ["train","test"]:
      ...
      # fit splines for fractioning
      splines = {}
      masses = list(fractions.keys())
      for transformed_from in unique:
        fractions_to = [fractions[transformed_to][transformed_from] for transformed_to in unique]
        splines[transformed_from] = UnivariateSpline(masses, fractions_to, s=0, k=1)
        #x = np.linspace(min(unique),max(unique),num=10)
        #y = splines[transformed_from](x)
        #print(transformed_from)
        #print(masses)
        #print(fractions_to)
        #print(x)
        #print(y)
      ...
      flattening_spline = UnivariateSpline([(bins[i]+bins[i+1])/2 for i in range(len(bins)-1)], hist, s=0, k=3)
