...
class MultiDimMetrics():
  ...
  def __init__(self, network, parameters, sim_files, synth_fraction=1.0):
  ...
  def MakeDatasets(self, sim_file, only_synth=False, only_sim=False, synth_size="length"):

    if not only_synth:

      if self.verbose:
        print(f" - Making simulated datasets from:")
        print(sim_file)
      ...
      self.sim_dataset = sim_dp.GetFull("dataset", functions_to_apply=["untransform"])      

    if not only_sim:

      if self.verbose:
        print(f" - Making synthetic datasets")

      if synth_size == "length":
        n_sampled_events = int(len(self.sim_dataset)*self.synth_fraction)
      elif synth_size == "n_eff" or (synth_size == "n_eff_conditions" and len(self.parameters["Y_columns"]) == 0):
        sim_n_eff = np.sum(self.sim_dataset.loc[:,"wt"])**2 / np.sum(self.sim_dataset.loc[:,"wt"]**2)
        n_sampled_events = int(sim_n_eff*self.synth_fraction)

      # Get synthetic data
      if len(self.parameters["Y_columns"]) == 0:

        synth_dp = DataProcessor(
          [[partial(self.network.Sample, pd.DataFrame([]))]],
          "generator",
          n_events = n_sampled_events, # this is irrelevant
          options = {
            "parameters" : self.parameters,
          }
        )
        self.synth_dataset = synth_dp.GetFull(method="dataset")

      else:

        # Get conditions to sample over
        unique_rows = self.sim_dataset.loc[:,self.parameters["Y_columns"]].drop_duplicates()
        if len(unique_rows) < 100:
          unique_rows.reset_index(drop=True, inplace=True)
          unique_rows['sum_wt'] = unique_rows.apply(
            lambda row: self.sim_dataset.loc[
                self.sim_dataset[self.parameters["Y_columns"]].eq(row).all(axis=1), 'wt'
            ].sum(),
            axis=1
          )
          if synth_size == "n_eff_conditions":
            unique_rows['sum_wt_squared'] = unique_rows.loc[:,self.parameters["Y_columns"]].apply(
              lambda row: self.sim_dataset.loc[
                  self.sim_dataset[self.parameters["Y_columns"]].eq(row).all(axis=1), 'wt'
              ].pow(2).sum(),
              axis=1
            )
            unique_rows['n_eff'] = unique_rows['sum_wt']**2 / unique_rows['sum_wt_squared']
            n_sampled_events = int(np.sum(unique_rows['n_eff'])*self.synth_fraction)
            probs = unique_rows.loc[:,"n_eff"].to_numpy()
          else:
            probs = unique_rows.loc[:,"sum_wt"].to_numpy()
          probs /= np.sum(probs)
          indices = np.random.choice(len(unique_rows), size=n_sampled_events, replace=True, p=probs)
          synth_conditions = unique_rows.iloc[indices].loc[:,self.parameters["Y_columns"]]
          synth_wts = 1/probs[indices]
        else:
          print("WARNING: Removing negative weights, as setting conditions correctly would take too long.")
          probs = self.sim_dataset.loc[:,"wt"].to_numpy()
          probs[probs<0] = 0
          probs /= np.sum(probs)
          indices = np.random.choice(len(self.sim_dataset), size=n_sampled_events, replace=True, p=probs)
          synth_conditions = self.sim_dataset.iloc[indices].loc[:,self.parameters["Y_columns"]]

        # Run loop
        indices_ranges = np.array_split(np.arange(n_sampled_events), np.floor(n_sampled_events/self.batch_size)+1)
        first = True
        #count = 0
        for indices in indices_ranges:
          #if self.verbose:
          #  print(f"   - {count}/{n_sampled_events} events")
          #count += len(indices)

          synth_dp = DataProcessor(
            [[partial(self.network.Sample, synth_conditions.iloc[indices])]],
            "generator",
            n_events = 1, # this is irrelivant
            options = {
              "parameters" : self.parameters,
            }
          )
          synth_dataset = synth_dp.GetFull(method="dataset")
          if first:
            self.synth_dataset = synth_dataset
            first = False
          else:
            self.synth_dataset = pd.concat([self.synth_dataset, synth_dataset], ignore_index=True)

      # Add conditions to dataset
      for col in self.parameters["Y_columns"]:
        self.synth_dataset.loc[:,col] = synth_conditions.loc[:,col].to_numpy()

      # Add weights for synth
      self.synth_dataset.loc[:,"wt"] = 1.0

      # Normalise conditions
      if len(self.parameters["Y_columns"]) != 0:
        if len(unique_rows) < 100:
          self.synth_dataset.loc[:,"wt"] = synth_wts
  ...
  def Run(self, seed=42, make_datasets=True):
    ...
    for sim_file_name, sim_file in self.sim_files.items():
      ...
      if make_datasets:
        self.MakeDatasets(sim_file)

      if "BDT Separation" in self.metrics:
        auc, accuracy = self.DoBDTSeparation()
        metrics[f"bdt_auc_{sim_file_name}"] = auc
        metrics[f"bdt_accuracy_{sim_file_name}"] = accuracy
      if "K-Means Chi Squared" in self.metrics:
        metrics[f"k_means_chi_squared_{sim_file_name}"] = self.DoKMeansChiSquared()
      if "Wasserstein Sliced" in self.metrics:
        metrics[f"wasserstein_sliced_{sim_file_name}"] = self.DoWassersteinSliced()
      if "Wasserstein Unbinned" in self.metrics:
        metrics[f"wasserstein_unbinned_{sim_file_name}"] = self.DoWassersteinUnbinned()
