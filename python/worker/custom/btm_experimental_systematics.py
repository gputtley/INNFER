...
def btm_jec(
  ...
  for name, info in jec_uncert.items():
    ...
    for ind in range(len(syst_names)):
      ...
      if include_met_syst:
        df[f"MET_shiftFactor_{syst_names[ind]}"] = 1.0
        added_columns.append(f"MET_shiftFactor_{syst_names[ind]}")
      ...
      if include_met_syst:
        met_indices = selected_indices.copy()
      ...
      if info["Type"] == "corrFactor":
        ...
        if include_met_syst:
          df.loc[met_indices, f"MET_shiftFactor_{syst_names[ind]}"] = 1.0 + (scalings * df.loc[met_indices, syst_names[ind]] * (df.loc[met_indices, f"MET_corrFactor_{name}"] - 1.0))
        ...
        if include_met_syst:
          df.loc[met_indices, f"MET_shiftFactor_{syst_names[ind]}"] = 1.0 + ((df.loc[met_indices,syst_names[ind]]>=0) * scalings * df.loc[met_indices, syst_names[ind]] * (df.loc[met_indices, "MET_smearFactor_up"] - 1.0)) + ((df.loc[met_indices,syst_names[ind]]<0) * scalings * abs(df.loc[met_indices, syst_names[ind]]) * (df.loc[met_indices, "MET_smearFactor_down"] - 1.0))
      ...
      if include_met_syst:
        df["MET_pt"] *= df[f"MET_shiftFactor_{syst_names[ind]}"]
