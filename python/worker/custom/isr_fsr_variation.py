...
def isr_fsr_weight(
  ...
  nu_fsr = df["log_fsr"]/np.log(2)
  nu_isr = df["log_isr"]/np.log(2)
  ...
  df["GenWeights_isr0p5fsr1"] = df["GenWeights_isr0p5fsr1"].clip(lower=lower_clip, upper=higher_clip)
  df["GenWeights_isr2fsr1"] = df["GenWeights_isr2fsr1"].clip(lower=lower_clip, upper=higher_clip)

  fsr_asymln = np.clip(AsymLogNormal(nu_fsr, kp=df["GenWeights_isr1fsr2"], km=df["GenWeights_isr1fsr0p5"]), 0.0, 10.0)
  isr_asymln = np.clip(AsymLogNormal(nu_isr, kp=df["GenWeights_isr2fsr1"], km=df["GenWeights_isr0p5fsr1"]), 0.0, 10.0)

  df.loc[:, "wt"] *= fsr_asymln * isr_asymln
