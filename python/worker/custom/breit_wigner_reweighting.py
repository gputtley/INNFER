...
def bw_reweight(df, mass_to="bw_mass", mass_from="sim_mass", gen_mass="GenTop1_mass"):
  df.loc[:,"wt"] *= bw(df.loc[:,mass_to], df.loc[:,gen_mass]) / bw(df.loc[:,mass_from], df.loc[:,gen_mass])
  return df
...
def bw_fractions(df, spline_locations="spline_locations.yaml", mass_to="bw_mass", mass_from="sim_mass", category="run2"):
