    ...
    for poi in cfg["pois"]:
        ...
        for other_poi_values in list(product(*unique_values_for_other_pois)):
            ...
            if len(poi_freeze.keys()) > 0:
                poi_freeze_name = "_for_" + "_".join(
                    [f"{k}_eq_{str(v).replace('.', 'p')}" for k, v in poi_freeze.items()])
            else:
                poi_freeze_name = ""
            poi_loop.append({
                "poi": poi,
    ...
    if not "all" in parameters["yield"].keys():
        ...
        for ind in range(len(parameters["Y_columns"])):
            orig_dict[parameters["Y_columns"][ind]] = sorted(
                list(set([float(k.split("_")[ind].replace("p", ".").replace("m", "-")) for k in
                          parameters["yield"].keys()])))
            ...
            for poi in mesh_pois:
                for nuisance in nuisance_loop:
                    ...
                    for nuisance_val in orig_dict[nuisance]:
                        parameters["yield"] = GetVariedRowValue(poi_vars, nuisance_vars, parameters, poi, nuisance,
                                                                nuisance_val,
...
def DeleteSubOptimalFiles(folder_path, type, min_val, name="Gaussian"):
...
def CleanFiles(cfg, name="Gaussian"):
    ...
    for root, dirs, files in os.walk(f"models/{cfg['name']}/"):
        for file in files:
            if file.endswith(".h5") and file.startswith(name):
                if is_float(file[-10:-3]):
                    vals.append(float(file[-10:-3]))

        min_val = format(min(vals), '.5f')  # find the minimum

        # Delete suboptimal files
        DeleteSubOptimalFiles(folder_path_architecture, 'yaml', min_val, name=name)
        DeleteSubOptimalFiles(folder_path, 'h5', min_val, name=name)

    # Rename optimal files
    file_paths = [[f"{folder_path}{name}_{min_val}.h5", f"{folder_path}{name}.h5"],
                  [f"{folder_path_architecture}{name}_architecture_{min_val}.yaml",
                   f"{folder_path_architecture}{name}_architecture.yaml"],
                  ]
    for filecombo in file_paths:
        if os.path.exists(filecombo[0]):
