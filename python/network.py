...
class Network():
    ...
    def BuildModel(self):
        ...
        if self.fix_1d_spline:
            print(
                "WARNING: Running fix for 1D latent space for spline and interleaved couplings. Code may take longer to run.")
            latent_dim = 2
    ...
    def ProbabilityIntegral(self, Y, y_columns=None, n_integral_bins=10000, n_samples=10 ** 4, ignore_quantile=0.000,
                            extra_fraction=0.25, method="histogramdd", verbose=True):
        """
        ...
        if method == "histogramdd":
            ...
            for col in range(synth.shape[1]):
                ranges.append(
                    [np.quantile(synth[:, col], ignore_quantile), np.quantile(synth[:, col], 1 - ignore_quantile)])
