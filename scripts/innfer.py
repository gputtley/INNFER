...
for file_name, parquet_name in cfg["files"].items():
  ...
  if args.step in ["PreProcess","Train"] and args.submit is not None:
    ...
    options = {"submit_to": args.submit, "cmds": [cmd], "job_name": f"jobs/{cfg['name']}/{args.step.lower()}/innfer_{args.step.lower()}_{cfg['name']}_{file_name}.sh", "sge_queue":args.sge_queue}
    from batch import Batch
    sub = Batch(options=options)
    sub.Run()
    continue
      ...
      for ind, info in enumerate(GetValidateLoop(cfg, parameters[file_name])):
        ...
        if args.step == "ValidateGeneration":
          ...
          if args.submit is not None:
            ...
            options = {"submit_to": args.submit, "cmds": [cmd], "job_name": f"jobs/{cfg['name']}/{args.step.lower()}/innfer_{args.step.lower()}_{cfg['name']}_{file_name}_{ind}.sh", "sge_queue":args.sge_queue}
            from batch import Batch
            sub = Batch(options=options)
            sub.Run()
            continue
          ...
          if args.submit is not None and not args.sub_step == "Scan":
            cmd = f"python3 {' '.join([i for i in sys.argv if '--submit' not in i and '--specific-file' not in i and '--specific-val-ind' not in i])} --specific-file={file_name} --specific-val-ind={ind}"
            options = {"submit_to": args.submit, "cmds": [cmd], "job_name": f"jobs/{cfg['name']}/{args.step.lower()}/{args.sub_step.lower()}/innfer_{args.step.lower()}_{args.sub_step.lower()}_{cfg['name']}_{file_name}_{ind}.sh", "sge_queue":args.sge_queue}
            from batch import Batch
            sub = Batch(options=options)
            sub.Run()
            continue
              ...
              for point_ind, scan_value in enumerate(scan_values_info["scan_values"]):
                ...
                if ((point_ind+1) % args.scan_points_per_job == 0) or (point_ind == len(scan_values_info["scan_values"])-1):
                  ...
                  if args.submit is not None:
                    ...
                    options = {"submit_to": args.submit, "cmds": [cmd], "job_name": f"jobs/{cfg['name']}/{args.step.lower()}/{args.sub_step.lower()}/innfer_{args.step.lower()}_{args.sub_step.lower()}_{cfg['name']}_{file_name}_{ind}_{job_ind}.sh", "sge_queue":args.sge_queue}
                    from batch import Batch
                    sub = Batch(options=options)
                    sub.Run()
                  job_ind += 1
        ...
        for point_ind, scan_value in enumerate(scan_values_info["scan_values"]):
          ...
          if ((point_ind+1) % args.scan_points_per_job == 0) or (point_ind == len(scan_values_info["scan_values"])-1):
            ...
            if args.submit is not None:
              ...
              options = {"submit_to": args.submit, "cmds": [cmd], "job_name": f"jobs/{cfg['name']}/{args.step.lower()}/{args.sub_step.lower()}/innfer_{args.step.lower()}_{args.sub_step.lower()}_{cfg['name']}_combined_{ind}_{job_ind}.sh", "sge_queue":args.sge_queue}
              from batch import Batch
              sub = Batch(options=options)
              sub.Run()
            job_ind += 1
